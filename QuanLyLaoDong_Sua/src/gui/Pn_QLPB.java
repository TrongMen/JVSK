...
public class Pn_QLPB extends JPanel implements ActionListener,MouseListener{
	...
	public Pn_QLPB(){
		...
		b3.add(lbSLNV = new JLabel("Số lượng nhân viên:"));
		lbSLNV.setFont(new Font("Segoe UI", Font.PLAIN, 16));
		b3.add(txSLNV = new JTextField(30));
		...
		String[] header = "Mã phòng ban;Phòng ban;Số lượng nhân viên".split(";");
		...
		if(o.equals(btnThem)) {
			...
			if(row>=0) {
				...
				if(dspb.update(pb)) {
					...
					table.setValueAt(txSLNV.getText(), row,2);
